// Rev62 nominal CAD geometry, millimetres, Z = motor axis.
// Editable in OpenSCAD. STEP files in this package contain exact analytic solids.
// The bobbin lands and end flanges are provisional mechanical additions.
...
/* [Reference envelope, no assigned physical material] */
reference_id = 17;
reference_od = 23;
...
assert(bobbin_id>tube_od && coil_id>bobbin_id);
...
module winding_pack() {
    for(i=[0:coil_count-1]) color([[0.87,0.47,0.17],[0.75,0.36,0.10],[0.95,0.62,0.25]][i%3])
...
module magnetic_stack() {
    for(i=[0:magnet_pairs-1]) {
        ...
        // Each pair has the same axial magnetization; adjacent pairs alternate.
